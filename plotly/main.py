...
def ellipse(major_axis, eccentricity):
    tracex = []
    tracey = []
    for i in range(360):
        radius = (major_axis*(1-(eccentricity**2)))/(1-(eccentricity*math.cos(i)))
        tracex += radius*np.cos(math.radians(i))
        tracey += radius*np.sin(math.radians(i))
    trace = go.Scatter3d(x=xcrd, y=ycrd, z=zcrd, marker=dict(size=0.1), line=dict(color=clr,width=wdth))
    return trace
